...
## Hard Stop
- Do not write into the `/home/.claude` home directory (Claude Code's own system/session storage). The user prefers persistent notes to live in this project's own `./.claude/` directory instead.
- Do not search across the entire `.claude/docs/repos` folder at once (e.g. `grep -r` over all of `docs/repos`). Target one specific repo subfolder at a time (e.g. `.claude/docs/repos/etched`, `.claude/docs/repos/Data-Anchor`).
